...
class EventUpdatesListener:
    def __init__(self, app):
        ...
        self.subscription_path = self.subscriber.subscription_path('miso-proyecto-de-grado-g09', 'food-data-events-sub')
    ...
    def callback(self, message):
        with self.app.app_context():  # Ensure Flask app context for DB session
            ...
            if message_data['type'] == 'FoodDataUpdated':
                self.process_food_data_updated(message_data)
    ...
    def process_food_data_updated(self, message):
        ...
        user.daily_calories = message['data'].get('daily_calories')
        user.daily_protein = message['data'].get('daily_protein')
        user.daily_liquid = message['data'].get('daily_liquid')
        user.daily_carbs = message['data'].get('daily_carbs')
        user.meal_frequency = message['data'].get('meal_frequency')
        
        try:
            db.session.commit()
            print(f"Food data updated for user {user.id}.")
        except Exception as e:
            db.session.rollback()
            print(f"Failed to update food data for user {user.id}: {e}")
